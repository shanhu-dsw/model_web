<template>
      ...
      <div class="create_box">
        <div class="create_title">
          <a style="color: #6e789a; padding-left: 20px" @click="back_list"
            >返回</a
          >
          <div style="font-size: 20px; font-weight: bold">编辑抓拍引擎</div>
          <el-button type="primary" size="" class="submit_form" @click="ModifyEngineCapture">提交</el-button>
        </div>
    ...
    <script>
    import { GetEngineCapture,ModifyEngineCapture } from '@/api/system';
  export default {
    data() {
      return {
        form: {
    ...
    methods: {
      ModifyEngineCapture(){
        ...
        if(valid){
          ModifyEngineCapture(this.form).then(res => {
      ...
      getEngineCapture(){
        ...
        GetEngineCapture(request).then(res => {
          if(res.data.status == "SUCCESS"){
