<template>
      ...
      <div class="create_box">
        <div class="create_title">
          <a style="color: #6e789a; padding-left: 20px" @click="back_list"
            >返回</a
          >
          <div style="font-size: 20px; font-weight: bold">新增识别引擎</div>
          <el-button type="primary" size="" class="submit_form" @click="AddEngineVision">提交</el-button>
        </div>
    ...
    <script>
    import { GetEngineCapture,AddEngineVision } from '@/api/system';
  export default {
    data() {
      return {
        form: {
    ...
    methods: {
      ...
      AddEngineVision(){
        ...
        if(valid){
          AddEngineVision(this.form).then((res) => {
            if (res.data.status == "SUCCESS") {
              this.$message({
                message: "新增成功",
                type: "success",
