  ...
  methods: {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          if (this.active > 0) {
            ...
            const cameras_form = this.CameraList.filter((item, index) => {
              return item.uuid == this.form.cameras_form[index];
            });
            const model_form = this.ModelList.filter((item, index) => {
              return item.uuid == this.form.model_form[index];
            });
            this.$refs.cameras_deploy.showModal(cameras_form, model_form); // 调用子组件的方法显示弹出框
          }
        }
      });
    },
    ...
    getLocationCamera() {
      ...
      GetLocationCamera(request).then((res) => {
        ...
        this.form.cameras_form = res.data.content.array.map((item) => {
          return {
            name: item.camera_name,
            uuid: item.camera_uuid,
    ...
    getLocationModel(){
        const request = {
          ...
          GetLocationModel(request).then((res) => {
            this.form.model_form = res.data.content.array.map((item) => {
              return {
